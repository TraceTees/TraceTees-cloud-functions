...
const SECRET_KEY = `projects/760049063210/secrets/${config.encryption.keyPath}`;
...
const getEncryptionKey = async (): Promise<Buffer> =>
  getEncryptionSecret(SECRET_KEY_DEFAULT_VERSION);
...
export const getAllEncryptionKeys = async (): Promise<Buffer[]> => {
  ...
  enabledVersions.sort((a, b) =>
    isDefaultKey(`${a.name}`) ? -1 : isDefaultKey(`${b.name}`) ? 1 : 0
  );
...
const isDefaultKey = (_: string) =>
  SECRET_KEY_DEFAULT_VERSION.substring(
    SECRET_KEY_DEFAULT_VERSION.length - 2
  ) === _.substring(_.length - 2);

async function getEncryptionSecret(
  keyPathIncludingVersion: string
): Promise<Buffer> {
  const secretManagerClient = new SecretManagerServiceClient();

  console.log(
    "getEncryptionSecret:",
    `Getting encryption key: ${keyPathIncludingVersion}`
  );
